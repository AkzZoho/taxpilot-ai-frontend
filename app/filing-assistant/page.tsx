...
import { AlertTriangle, CheckCircle2, ExternalLink, FileText, PartyPopper } from "lucide-react";

const checklist = [
  { item: "Form 16 uploaded", done: true },
  { item: "Extraction reviewed", done: true },
  { item: "Regime selected (New)", done: true },
  ...
  return (
    <AppShell>
      <div className="animate-fade-up">
        ...
        <p className="page-subtitle">You're almost there — complete these steps to file your ITR.</p>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-3">
          ...
          <Card className="flex flex-col">
            ...
            <p className="mt-3 text-xs text-slate-500 leading-5 flex-1">
              For salaried individuals with salary, one house property, and other sources (excluding capital gains).
            </p>
            <div className="mt-4 rounded-xl bg-amber-50 border border-amber-100 px-3 py-2 text-xs text-amber-700">
              Verify: no capital gains or business income.
            </div>
          </Card>
          ...
          <Card>
            ...
            <p className="mt-1.5 text-xs text-slate-400">
              {checklist.filter(c => c.done).length}/{checklist.length} steps done
            </p>
          </Card>
        ...
        {step === "guide" ? (
          <div className="mt-5 rounded-2xl border border-slate-100 bg-white p-5 shadow-soft flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                TaxPilot has prepared all your values. Head to incometax.gov.in to file using your summary.
              </p>
            </div>
            <Button size="lg" onClick={() => setStep("ready")} className="shrink-0">
              I&apos;m ready to file
            </Button>
          </div>
        ) : (
          <div className="mt-5 rounded-2xl border border-trust-200 bg-trust-50 p-6 text-center">
            ...
            <p className="text-base font-bold text-trust-800">Great work! You&apos;re all set.</p>
            <p className="mt-1 text-sm text-trust-700">
              Use your TaxPilot summary to complete filing on the official portal.
            </p>
            <a
